...
namespace Meatcorps.Game.CyberMaze.Particles;

public static class ExplosionParticle
{
    public static ParticleSystemBuilder GenerateParticleSystem(Texture2DItem<GameSprites> sprites, SoundFxManager<GameSounds> soundFx)
    {
        return new ParticleSystemBuilder().SetMaxParticles(50)
            ...
            .SetSpawnLogic(() => new Particle
            {
                ...
                OnStart = particle =>
                {
                    ...
                    soundFx.Play(GameSounds.Explode1, 1, 0.75f + Raylib.GetRandomValue(0, 100) / 400f);
                },
                OnDraw = particle =>
                {
                    sprites.DrawAnimationWithNormal(GameSprites.ExplosionAnimation, particle.NormalizedLifetime,
                        particle.Position, particle.Color, particle.Rotation, new Vector2(16, 16), particle.Size);
                }
            });
    }
}