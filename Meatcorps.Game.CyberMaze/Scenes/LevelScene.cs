...
public class LevelScene : BaseScene
{
    ...
    private void LoadLevel(List<List<char>> levelData)
    {
        ...
        if (_firstLevelData)
        {
            ...
            {
                AddGameObject(player.CyberPlayer);
            ...
            foreach (var gridItem in analyzer.IterateAll())
            {
                ...
                switch (character)
                {
                    case '1':
                        ...
                        AddGameObject(GhostFactory.Create(_level.ToWorldPosition(position), corner1, GhostType.Attacker,
                            _level,
                            _worldService));
                        break;
                    case '2':
                        analyzer.Search('6', out var corner2);
                        AddGameObject(GhostFactory.Create(_level.ToWorldPosition(position), corner2, GhostType.Another,
                            _level,
                            _worldService));
                        break;
                    case '3':
                        analyzer.Search('7', out var corner3);
                        AddGameObject(GhostFactory.Create(_level.ToWorldPosition(position), corner3, GhostType.RandomGhost,
                            _level,
                            _worldService));
                        break;
                    case '4':
                        analyzer.Search('8', out var corner4);
                        AddGameObject(GhostFactory.Create(_level.ToWorldPosition(position), corner4, GhostType.Scared,
                            _level,
                            _worldService));
                        break;
                }
    ...
    private void SpawnPlayer(int playerId, PointInt position)
    {
        ...
        CyberPlayer.Setup(_worldService, player, _level.ToWorldPosition(position));
        _players.Add(player);
    }
