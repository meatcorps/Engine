...
public class CyberPlayer: BasePlayer, ICollisionEventsFiltered
{
    ...
    protected override void OnInitialize()
    {
        ...
        Layer = 3;
        LevelData.TargetCyberPlayer = Player;
    ...
    protected override void OnDraw()
    {
        ...
        {
            if (!_diedSecondFase)
                Sprites.Draw(GameSprites.CyberPlayerDown1, position, Color.Black);
        }
        else
        {
            if (!LevelData.GhostScared)
            {
                if (facing == new PointInt(0, 1) || facing == new PointInt(0, 0))
                    Sprites.DrawAnimationWithNormal(GameSprites.CyberPlayerDownAnimation, _animationTimer.NormalizedElapsed,
                        position, Player.Color);
                if (facing == new PointInt(0, -1))
                    Sprites.DrawAnimationWithNormal(GameSprites.CyberPlayerUpAnimation, _animationTimer.NormalizedElapsed,
                        position, Player.Color);
                if (facing == new PointInt(1, 0))
                    Sprites.DrawAnimationWithNormal(GameSprites.CyberPlayerRightAnimation, _animationTimer.NormalizedElapsed,
                        position, Player.Color);
                if (facing == new PointInt(-1, 0))
                    Sprites.DrawAnimationWithNormal(GameSprites.CyberPlayerLeftAnimation, _animationTimer.NormalizedElapsed,
                        position, Player.Color);
            }
            else
            {
                if (facing == new PointInt(0, 1) || facing == new PointInt(0, 0))
                    Sprites.DrawAnimationWithNormal(GameSprites.SuperCyberPlayerDownAnimation,
                        _animationTimer.NormalizedElapsed, position, Player.Color);
                if (facing == new PointInt(0, -1))
                    Sprites.DrawAnimationWithNormal(GameSprites.SuperCyberPlayerUpAnimation,
                        _animationTimer.NormalizedElapsed, position, Player.Color);
                if (facing == new PointInt(1, 0))
                    Sprites.DrawAnimationWithNormal(GameSprites.SuperCyberPlayerRightAnimation,
                        _animationTimer.NormalizedElapsed, position, Player.Color);
                if (facing == new PointInt(-1, 0))
                    Sprites.DrawAnimationWithNormal(GameSprites.SuperCyberPlayerLeftAnimation,
                        _animationTimer.NormalizedElapsed, position, Player.Color);

                Sprites.DrawAnimationWithNormal(GameSprites.SuperCyberPlayerEffectAnimation,
    ...
    {
        player.CyberPlayer = new CyberPlayer(player);
        player.Body = world.RegisterRectFBody(player.CyberPlayer, new RectF(position, new SizeF(16, 16)));
    ...
    private void AllCollectiblesCollected()
    {
        ...
        Scene.AddGameObject(new Drone(Player.Body.Position, GameSprites.CyberPlayerDown1, () => Enabled = false, 1500, true, EndGame));
        ...
        foreach (var ghost in LevelData.Ghosts)
        {
            switch (ghost.Type)
            {
                case GhostType.Attacker:
                    Scene.AddGameObject(new Drone(ghost.Body.Position, GameSprites.CyberPlayerDown1, () => ghost.Enabled = false, 1000, true));
                    break;
                case GhostType.Another:
                    Scene.AddGameObject(new Drone(ghost.Body.Position, GameSprites.GhostAnother1, () => ghost.Enabled = false, 1100, true));
                    break;
                case GhostType.RandomGhost:
                    Scene.AddGameObject(new Drone(ghost.Body.Position, GameSprites.GhostStrange1, () => ghost.Enabled = false, 1200, true));
                    break;
                case GhostType.Scared:
                    Scene.AddGameObject(new Drone(ghost.Body.Position, GameSprites.GhostEmo1, () => ghost.Enabled = false, 1300, true));
                    break;
            }
        }
    }
        ...
        if (pair.TryGetOwner<Ghost>(out var ghost))
        {
            ...
            if (!LevelData.GhostScared)
            {
                ...
                foreach (var player in LevelData.Players)
                {
                    ...
                    {
                        Scene.AddGameObject(new Drone(player.Body.Position, GameSprites.CyberPlayerDown1, () => player.CyberPlayer.Enabled = false, 1500, true,
                            () => { }));
                    }
                }
