...
public class Map: ResourceGameObject, ICollisionEventsFiltered
{
    ...
    protected override void OnUpdate(float deltaTime)
    {
        ...
        if (canStart && !_started)
        {
            ...
            Scene.AddGameObject(new Drone(LevelData.Ghosts[0].Body.Position, GameSprites.GhostAttacker1, () =>
            {
                LevelData.Ghosts[0].Enabled = true;
            }, 2500));
            Scene.AddGameObject(new Drone(LevelData.Ghosts[1].Body.Position, GameSprites.GhostAnother1, () =>
            {
                LevelData.Ghosts[1].Enabled = true;
            }, 2510));
            Scene.AddGameObject(new Drone(LevelData.Ghosts[2].Body.Position, GameSprites.GhostStrange1, () =>
            {
                LevelData.Ghosts[2].Enabled = true;
            }, 2520));
            Scene.AddGameObject(new Drone(LevelData.Ghosts[3].Body.Position, GameSprites.GhostEmo1, () =>
            {
                LevelData.Ghosts[3].Enabled = true;
            }, 2530));
            var counter = 0;
            foreach (var pacMan in Scene.GetGameObjects<CyberPlayer>())
            {
                Scene.AddGameObject(new Drone(pacMan.Position, GameSprites.CyberPlayerDown1, () =>
                {
                    counter++;
                    pacMan.Enabled = true;
