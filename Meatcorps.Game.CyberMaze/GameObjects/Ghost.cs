...
public class Ghost : ResourceGameObject, ICollisionEvents
{
    ...
    protected override void OnUpdate(float deltaTime)
    {
        ...
        if (LevelData.TargetCyberPlayer is null)
            return;
    ...
    {
        if (pair.TryGetOwner<CyberPlayer>(out var _) && pair.TryGetOwner<PowerPellet>(out var _))
        {
            _ghostState.ResetTimers();
            return;
        }
            ...
            else
            {
                switch (_behaviour.Type)
                {
                    case GhostType.Attacker:
                        Sprites.DrawAnimationWithNormal(GameSprites.GhostAttackerAnimation,
                            animation,
                            position, Raylib.ColorAlpha(Color.White, 0.8f));
                        break;
                    case GhostType.Another:
                        Sprites.DrawAnimationWithNormal(GameSprites.GhostAnotherAnimation,
                            animation ,
                            position, Raylib.ColorAlpha(Color.White, 1f));
                        break;
                    case GhostType.RandomGhost:
                        Sprites.DrawAnimationWithNormal(GameSprites.GhostStrangeAnimation,
                            animation,
                            position, Raylib.ColorAlpha(Color.White, 1f));
                        break;
                    case GhostType.Scared:
                        Sprites.DrawAnimationWithNormal(GameSprites.GhostEmoAnimation,
                            animation,
                            position, Raylib.ColorAlpha(Color.White, 1f));
                        break;
                }
