    ...
    public static ParticleSystemBuilder GenerateParticleSystem(Texture2DItem<GameSprites> sprites)
    {
        return new ParticleSystemBuilder().SetMaxParticles(50)
            ...
            .SetSpawnLogic(() => new Particle
            {
                ...
                OnDraw = particle =>
                {
                    sprites.DrawAnimationWithNormal(GameSprites.ExplosionAnimation, particle.NormalizedLifetime, particle.Position, particle.Color, particle.Rotation, new Vector2(16, 16), particle.Size);
                }
            });
    }
}