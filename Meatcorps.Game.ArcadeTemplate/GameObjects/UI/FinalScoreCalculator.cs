...
public class FinalScoreCalculator: ResourceGameObject
{
    ...
    protected override void OnInitialize()
    {
        ...
        if (highestScore < finalScore)
            pointsWon += 1000;
        _relativePoints = pointsWon - _gameInfo.PricePoints;
        ...
        if (_relativePoints != 0)
            if (_relativePoints > 0 )
                ListValues("Points won:", _relativePoints, 0, "!", 16);
            else 
                ListValues("Points lost:", _relativePoints, 0, "!", 16);

        _iterateTimer = new TimerOn(_itemsInList * 1000);
        for (var i = 0; i < _itemsInList; i++)
            SetLineOnOrOff(i, false);
        
        _arcadePointMutator.SubmitPoints(_playerId, pointsWon);
