...
public class IntroUI : ResourceGameObject, IIntroSlide
{
        ...
        if (_messageTimer.Output && _introScene.TotalPlayersReady == 0)
        {
            ...
            switch (_counter)
            {
                case 1:
                    ...
                    break;
                case 2:
                    targetText = "KILLS THE SKULLS!";
                    break;
                case 3:
                    targetText = "THE MOST SCARY SMASH GAME EVER!";
                    break;
            }
            ...
            _uiMessage.Show(targetText, new UIMessageStyle
            {
                ...
                Style = TextKitStyles.HudDefault(Fonts.GetFont()) with
                {
                    PixelOutline = true, OutlineColor = Color.Black, UseOutline = true
                }
            });
        }
    ...
    protected override void OnDraw()
    {
        ...
        Raylib.DrawTextEx(Fonts.GetFont(), _introScene.TotalPlayersReady > 0 ? "READY?" : _game.Name,
            new Vector2(startPosX + 24, startPosY + 100), 32, 0, colorTitle);

        if (_introScene.TotalPlayersReady == 0 || _fastTimer.NormalizedElapsed > 0.5f)
        {
            Sprites.DrawAnimationWithNormal(GameSprites.ArcadeButtonAnimation, _buttonTimer.NormalizedElapsed,
                new Vector2(startPosX + 16, startPosY + 172), Color.LightGray);
            Raylib.DrawTextEx(Fonts.GetFont(), "START TO KILL!", new Vector2(startPosX + 64, startPosY + 180), 16, 0,
                Color.Red);
        }


        Raylib.DrawRectangleGradientV(0, RenderTarget!.RenderHeight - 64, RenderTarget!.RenderWidth, 80,
