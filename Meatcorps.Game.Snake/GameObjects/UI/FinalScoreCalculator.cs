...
public class FinalScoreCalculator: SnakeGameObject
{
    ...
    private readonly SessionDataBag<SnakePlayerData> _playerSessionData;
    ...
    public FinalScoreCalculator(Rect bounds, int playerId)
    {
        ...
        GlobalObjectManager.ObjectManager.Get<SessionService<SnakeSessionData, SnakePlayerData>>()!
    ...
    protected override void OnInitialize()
    {
        ...
        _inlineRender.AddLabel(Fonts.GetFont(), null, _playerCheckin.GetPlayerName(_playerId), 16, _playerId == 1 ? Color.Red : Color.Blue).AddNewLine()
            .AddLabel(Fonts.GetFont(), null, "LET'S SUM IT UP!", 10, _playerId == 1 ? Color.Red : Color.Blue).AddSpacer(8, 16).AddNewLine();

        var highestScoreObject = GlobalObjectManager.ObjectManager.Get<HighScoreService>()!.GetScores().FirstOrDefault();
        ...
        if (_relativePoints != 0)
        {
            if (_relativePoints > 0)
                ...
                ListValues("Points lost:", _relativePoints, _gameInfo.PricePoints, "you idiot", 16);
        }
        else
        {
    ...
    private void ListValues(string label, float value, float from = 0, string afterFix = "", int size = 8)
    {
        ...
        _inlineRender
            ...
            .AddSmoothValue(Fonts.GetFont(), $"{_itemsInList}_2", smoothValue, size + 2, from < value ? Color.Green : Color.Red);
        ...
        if (currentLine != _previousLine)
        {
            if (currentLine == _itemsInList - 1)
            {
                if (_relativePoints > 0)
                    ...
                    Sounds.Play(SnakeSounds.Alarm, 1, 0.5f);

                GlobalObjectManager.ObjectManager.Get<MusicManager<SnakeMusic>>()!.Stop();
            } else 
