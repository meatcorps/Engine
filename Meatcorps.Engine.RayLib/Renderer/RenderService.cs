...
public class RenderService
{
    ...
    private readonly Dictionary<IRenderTargetStrategy, List<List<List<BaseGameObject>>>> _gameObjects = new();
    ...
    /// <summary>
    ///     Set render targets. If the renderTargetStrategies is null. It will load the ones from the GlobalObjectManager.
    ...
    public void RegisterRender(BaseGameObject gameObject)
    {
        if (gameObject.Scene.Layer >= _sceneLayers)
            throw new Exception(
                $"Scene layer out of bounds, scene layer: {gameObject.Scene.Layer} max: {_sceneLayers}");
