...
public sealed class UIMessageEmitter : BaseGameObject
{
        ...
        _particleSystem
            ...
            .SetSpawnLogic(() =>
            {
                var particle = new Particle
                {
                    ...
                    OnEnd = p =>
                    {
                        if (_payloadPool.TryDequeue(out var payload)) _particleSystem.Emit(1, null, null, payload);
    ...
    public void Show(string text, UIMessageStyle? message = null)
    {
        ...
        var totalDuration = (float)message.AppearDurationInMilliseconds + message.DisappearDurationInMilliseconds +
                            message.HoldDurationInMilliseconds;
        ...
        var positionFrom =
            ...
            UIAnchorHelper.ResolveAnchorPixel(UIAnchorHelper.InvertAnchor(message.AnchorFrom), textSizeFrom.X,
                textSizeFrom.Y) +
        ...
        var positionAfter =
            ...
            UIAnchorHelper.ResolveAnchorPixel(UIAnchorHelper.InvertAnchor(message.AnchorAfter), textSizeAfter.X,
                textSizeAfter.Y) +
            UIAnchorHelper.AnchorToDirectionVector2(message.AnchorAfter, message.PaddingTopBottomLeftDown);

        if ((message.AnchorTo is Anchor.Bottom or Anchor.BottomLeft or Anchor.BottomRight &&
             message.AnchorFrom is Anchor.BottomLeft or Anchor.BottomRight) ||
            (message.AnchorTo is Anchor.Top or Anchor.TopLeft or Anchor.TopRight &&
             message.AnchorFrom is Anchor.TopLeft or Anchor.TopRight))
            positionFrom.Y = positionTo.Y;

        if ((message.AnchorTo is Anchor.Bottom or Anchor.BottomLeft or Anchor.BottomRight &&
             message.AnchorAfter is Anchor.BottomLeft or Anchor.BottomRight) ||
            (message.AnchorTo is Anchor.Top or Anchor.TopLeft or Anchor.TopRight &&
             message.AnchorAfter is Anchor.TopLeft or Anchor.TopRight))
    ...
    public void Countdown(int durationInMilliseconds, UIMessageStyle? style = null, UIMessageStyle? goStyle = null,
        string finalText = "GO!")
    {
        if (style == null)
            style = UIMessagePresets.Countdown(_defaultStyle.Style.Font);

        style.HoldDurationInMilliseconds = durationInMilliseconds / 3 - style.DisappearDurationInMilliseconds -
                                           style.AppearDurationInMilliseconds;

        if (style.HoldDurationInMilliseconds < 0)
            throw new ArgumentException(
                $"DisappearDurationInMilliseconds and AppearDurationInMilliseconds together must be less then {durationInMilliseconds / 3} milliseconds");
        ...
        {
            goStyle = style;
            goStyle.HoldDurationInMilliseconds =
                1000 - style.DisappearDurationInMilliseconds - style.AppearDurationInMilliseconds;
        }
